...
pub fn process(input: &str) -> std::result::Result<String, Error> {
    ...
    for start_position in &terrain.start_positions {
        let hike_rating = hike(&terrain, start_position);
        score += hike_rating.len();
    }
    Ok(score.to_string())
}

fn hike(terrain: &Terrain, current_position: &UVec2) -> Vec<UVec2> {
    ...
    for offset in CARDINAL_OFFSETS {
        ...
        if let Some(next_cell) = terrain.get(&next_position) {
            ...
            if next_cell.cmp(&target_height).is_eq() {
                if target_height == 9 {
                    to_return.push(next_position.as_uvec2());
                    continue;
                }
